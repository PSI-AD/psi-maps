...
const FilteredProjectsCarousel: React.FC<FilteredProjectsCarouselProps> = ({
    ...
    return (
        <>
            {/* ── Desktop: Collapse toggle button — always visible, even when AI chat hides the list ── */}
            <button
                onClick={(e) => { e.stopPropagation(); setIsCollapsed(!isCollapsed); }}
                aria-label={isCollapsed ? 'Show property panel' : 'Collapse property panel'}
                className={`
                hidden md:flex fixed z-[4001] w-11 h-16
                bg-white/95 backdrop-blur-xl border border-slate-200/80 rounded-r-2xl
                items-center justify-center shadow-[4px_0_15px_rgba(0,0,0,0.06)]
                hover:bg-slate-50 transition-all duration-700 ease-in-out
            `}
                style={{
                    top: '50%',
                    transform: 'translateY(-50%)',
                    left: (isCollapsed || isAiChatOpen) ? '0px' : 'calc(24px + 360px)',
                }}
                title={isCollapsed ? 'Show panel' : 'Hide panel'}
            >
                <ChevronLeft className={`w-5 h-5 text-slate-500 transition-transform duration-300 ${(isCollapsed || isAiChatOpen) ? 'rotate-180' : ''}`} />
            </button>
